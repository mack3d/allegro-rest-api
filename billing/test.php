...
$gte = '&occurredAt.gte='.date('Y-m-d\T\00:\00:\00.\0\0\0\Z',strtotime("-5 day", strtotime($data)));
...
while (true){
    ...
    if ($offset > $pay->totalCount){break;}
    $i++;
}
usort($payment, "cmpw");
krsort($payment);
...
foreach($wplaty as $wplata){
    ...
    if (isset($wplata->surcharge->id)){$trid = trid($wplata->surcharge->id);}
