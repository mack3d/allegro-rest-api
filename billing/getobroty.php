...
function pobierz($allegro, $limit,$offset,$lte,$gte){
    $payment = $allegro->payments("GET", '/payment-operations?group=INCOME&limit='.$limit.'&offset='.$offset.$gte.$lte);
    return json_decode($payment);
}

$payment = pobierz($allegro, 1,0,$lte,$gte);
