...
class Returns
{
    ...
    {
        if (!empty($customerReturns) || $customerReturns != null) {
            $customerReturns = $customerReturns[0];
    ...
    protected function inpostApi()
    {
        try {
            ...
            $this->data = $json_data;
            if (isset($json_data->status)) $this->status = strtolower($json_data->status);
            if (isset($json_data->tracking_details)) $this->datetime = date("d.m.Y", strtotime($json_data->tracking_details[0]->datetime));
        } catch (Exception $e) {
            $this->status = '';
            $this->datetime = '';
        }
